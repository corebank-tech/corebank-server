...
public class SignupCompletionTransactionService {
    ...
    private static final String EXISTING_BANK_CUSTOMER_UNIQUE_KEY = "uk_customer_existing_bank_customer_id";
    ...
    @Transactional
    public RegisteredCustomer register(SignupCompletionSnapshot snapshot, LocalDateTime joinedAt) {
        RegisteredCustomer customer = registerCustomer(new RegisterCustomerCommand(
        ...
        termsAgreementPort.saveAll(customer.customerId(), snapshot.signup().agreedTerms(), joinedAt);
        accountRegistration.registerAll(new RegisterExistingAccountsCommand(
                customer.customerId(),
                snapshot.accounts().stream().map(this::toAccountData).toList()));
        return customer;
    }

    // 원장 고객 선검증과 INSERT 사이의 시간차 경합(TOCTOU)은 UK 제약만 막는다.
    // 그 위반을 선검증과 같은 ATH0303 으로 변환해 500 으로 새어나가지 않게 한다.
    private RegisteredCustomer registerCustomer(RegisterCustomerCommand command) {
        try {
            return customerRegistration.register(command);
        } catch (DataIntegrityViolationException exception) {
            if (violatesExistingBankCustomerKey(exception)) {
                throw new BusinessException(SignupErrorCode.DUPLICATE_EXISTING_BANK_CUSTOMER, exception);
            }
    ...
    private boolean violatesExistingBankCustomerKey(DataIntegrityViolationException exception) {
        if (!(exception.getCause() instanceof ConstraintViolationException violation)) {
            return false;
        }

        String constraintName = violation.getConstraintName();
        return constraintName != null && constraintName.contains(EXISTING_BANK_CUSTOMER_UNIQUE_KEY);
    }

    private RegisterExistingAccountsCommand.AccountData toAccountData(ExistingBankAccountSnapshot account) {
