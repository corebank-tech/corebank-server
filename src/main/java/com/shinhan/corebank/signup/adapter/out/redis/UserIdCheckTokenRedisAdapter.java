...
public class UserIdCheckTokenRedisAdapter implements UserIdCheckTokenPort {
    ...
    public UserIdCheckTokenRedisAdapter(StringRedisTemplate redisTemplate, ObjectMapper objectMapper) {
    ...
    public void save(String token, UserIdCheckTokenPayload payload, Duration ttl) {
        try {
            ...
            throw new IllegalStateException("아이디 중복확인 토큰 직렬화에 실패했습니다.", exception);
        }
    }
    ...
    private Optional<UserIdCheckTokenPayload> deserialize(String json) {
        ...
        try {
            return Optional.of(objectMapper.readValue(json, UserIdCheckTokenPayload.class));
        } catch (JacksonException exception) {
            throw new IllegalStateException("아이디 중복확인 토큰 역직렬화에 실패했습니다.", exception);
        }
    }
