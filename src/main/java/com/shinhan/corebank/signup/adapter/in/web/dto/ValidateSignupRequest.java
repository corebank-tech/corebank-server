...
public record ValidateSignupRequest(
        @Schema(
                description = "약관 동의 검증 후 발급된 1회성 인증 토큰",
                example = "TERMS_AUTH_7xP9qK2RmY5vLw8Z",
                nullable = true
        )
        String termsAuthToken,

        @Schema(
                description = "실명·계좌 인증 후 발급된 1회성 인증 토큰",
                example = "ACCOUNT_AUTH_7xP9qK2RmY5vLw8Z",
                nullable = true
        )
        String accountAuthToken,

        @Schema(
                description = "아이디 중복확인 후 발급된 1회성 토큰",
                example = "USER_ID_CHECK_7xP9qK2RmY5vLw8Z",
                nullable = true
        )
        String userIdCheckToken,

        @Schema(
                description = "이메일 인증 후 발급된 1회성 인증 토큰",
                example = "EMAIL_AUTH_7xP9qK2RmY5vLw8Z",
                nullable = true
        )
        ...
        @Schema(description = "가입 이메일", example = "user@mail.com", maxLength = 100)
        @NotBlank
        @Email
        @Size(max = 100)
        String email,
        ...
        @Schema(
                description = "휴대폰 번호. 하이픈 없는 숫자 11자리",
                example = "01012345678",
                pattern = "^\\d{11}$"
        )
        @NotBlank
