...
@Tag(name = "약관", description = "회원가입 약관 조회·동의 검증 API")
public class SignupTermsController {
    ...
    public SignupTermsController(
            GetSignupTermsUseCase getSignupTermsUseCase, CheckTermsAgreementUseCase checkTermsAgreementUseCase) {
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "회원가입 약관 조회 성공")
    public ApiResponse<SignupTermsResponse> getTerms() {
        SignupTermsResponse response = new SignupTermsResponse(getSignupTermsUseCase.getSignupTerms().stream()
    ...
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "약관 동의 검증 성공"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "400",
                description = "`CMN0001` 요청 형식 또는 약관 정보 오류 · `ATH0006` 필수 약관 미동의",
                content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    public ApiResponse<TermsAuthTokenResponse> checkTerms(@Valid @RequestBody CheckTermsAgreementRequest request) {
        TermsAgreementResult result = checkTermsAgreementUseCase.checkTermsAgreement(request.toCommand());

        return ApiResponse.success(TermsAuthTokenResponse.from(result));
    }
}
