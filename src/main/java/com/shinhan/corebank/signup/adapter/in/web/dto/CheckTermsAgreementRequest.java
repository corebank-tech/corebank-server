...
public record CheckTermsAgreementRequest(
        @Schema(description = "검증할 회원가입 약관 동의 목록") @NotEmpty List<@NotNull @Valid AgreedTermRequest> agreedTerms) {

    public CheckTermsAgreementCommand toCommand() {
        return new CheckTermsAgreementCommand(
                agreedTerms.stream().map(AgreedTermRequest::toCommand).toList());
    }
}
