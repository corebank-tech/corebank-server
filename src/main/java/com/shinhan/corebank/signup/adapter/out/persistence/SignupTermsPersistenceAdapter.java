...
@Component
public class SignupTermsPersistenceAdapter implements SignupTermsQueryPort {

    private final SignupTermsJpaRepository repository;

    public SignupTermsPersistenceAdapter(SignupTermsJpaRepository repository) {
        this.repository = repository;
    }

    @Override
    public List<SignupTerm> findLatestSignupTerms() {
        return repository.findLatestByTermsType(TermsType.SIGNUP).stream()
