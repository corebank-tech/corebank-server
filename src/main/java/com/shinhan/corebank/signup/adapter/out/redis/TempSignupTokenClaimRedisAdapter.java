...
@Component
public class TempSignupTokenClaimRedisAdapter
        implements TempSignupTokenClaimPort {
    ...
    public TempSignupTokenClaimRedisAdapter(
    ...
    @Override
    public Optional<TempSignupTokenPayload> claim(
            String token,
            String claimId
    ) {
        ...
        String json = redisTemplate.execute(
                CLAIM_SCRIPT,
                List.of(sourceKey(token), claimKey(token), ownerKey(token)),
                claimId
        );
    ...
    public void complete(String token, String claimId) {
        Long completed = redisTemplate.execute(
                COMPLETE_SCRIPT,
                List.of(claimKey(token), ownerKey(token)),
                claimId
        );
