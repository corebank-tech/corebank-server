...
public class EmailVerificationTokenRedisAdapter implements EmailVerificationTokenPort {
    ...
    public EmailVerificationTokenRedisAdapter(StringRedisTemplate redisTemplate, ObjectMapper objectMapper) {
    ...
    public void save(String token, EmailVerificationTokenPayload payload, Duration ttl) {
        try {
            ...
            throw new IllegalStateException("이메일 인증 토큰 직렬화에 실패했습니다.", exception);
        }
    }
    ...
    private Optional<EmailVerificationTokenPayload> deserialize(String json) {
        ...
        try {
            return Optional.of(objectMapper.readValue(json, EmailVerificationTokenPayload.class));
        } catch (JacksonException exception) {
            throw new IllegalStateException("이메일 인증 토큰 역직렬화에 실패했습니다.", exception);
        }
    }
