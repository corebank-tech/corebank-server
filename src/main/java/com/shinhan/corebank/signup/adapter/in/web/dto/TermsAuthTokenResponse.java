...
public record TermsAuthTokenResponse(
        ...
        @Schema(description = "약관 동의 인증 토큰 남은 유효시간(초)", example = "1800") long expiresIn) {

    public static TermsAuthTokenResponse from(TermsAgreementResult result) {
        return new TermsAuthTokenResponse(result.termsAuthToken(), result.expiresIn());
    }
}
