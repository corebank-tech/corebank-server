...
public record IssueEmailVerificationRequest(
        @Schema(description = "인증번호를 받을 이메일", example = "user@mail.com", maxLength = 100)
                @NotBlank
                @Email
                @Size(max = 100)
                String email,
        @Schema(description = "이메일 인증 목적", example = "SIGN_UP") @NotNull EmailVerificationPurpose purpose) {
