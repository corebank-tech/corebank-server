...
@Component
public class SignupTermsAgreementPersistenceAdapter
        implements SignupTermsAgreementPort {
    ...
    public SignupTermsAgreementPersistenceAdapter(
            SignupTermsJpaRepository termsRepository,
            CustomerTermsAgreementJpaRepository agreementRepository
    ) {
    ...
    private CustomerTermsAgreementJpaEntity toEntity(
            Long customerId,
            AgreedTerm agreedTerm,
            LocalDateTime agreedAt
    ) {
        Long termsId = parseTermsId(agreedTerm.termsId());
        termsRepository.findByTermsIdAndVersion(
                termsId,
                agreedTerm.version()
        ).orElseThrow(() -> new IllegalStateException(
                "동의한 약관 식별자와 버전이 현재 약관 정보와 일치하지 않습니다."
        ));
        return new CustomerTermsAgreementJpaEntity(
                customerId,
                termsId,
                agreedAt
        );
