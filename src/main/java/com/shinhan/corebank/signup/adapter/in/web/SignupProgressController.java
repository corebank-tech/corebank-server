...
public class SignupProgressController {
    ...
    public SignupProgressController(
            ValidateSignupUseCase validateSignupUseCase, GetSignupConfirmationUseCase getSignupConfirmationUseCase) {
    ...
    public ApiResponse<ValidateSignupResponse> validate(@Valid @RequestBody ValidateSignupRequest request) {
        return ApiResponse.success(ValidateSignupResponse.from(validateSignupUseCase.validate(request.toCommand())));
    }
    ...
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "마스킹된 확인정보 조회 성공"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "400",
    ...
    public ApiResponse<SignupConfirmationResponse> getConfirmation(
            @Parameter(description = "회원가입 입력 검증 후 발급된 임시 가입 토큰", required = true, example = "TEMP_SIGNUP_AbCdEf123456")
                    @RequestHeader(value = "X-Signup-Token", required = false)
                    String tempSignupToken) {
        return ApiResponse.success(
                SignupConfirmationResponse.from(getSignupConfirmationUseCase.getConfirmation(tempSignupToken)));
    }
}
