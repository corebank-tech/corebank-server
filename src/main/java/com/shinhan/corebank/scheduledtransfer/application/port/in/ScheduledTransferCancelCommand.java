...
@Builder
public record ScheduledTransferCancelCommand(
        Long customerId,
    ...
    public ScheduledTransferCancelCommand {
        ...
        if (scheduledTransferIds == null
                || scheduledTransferIds.isEmpty()
                || scheduledTransferIds.stream().anyMatch(Objects::isNull)) {
            throw new BusinessException(CommonErrorCode.REQUIRED_FIELD_MISSING);
        }
        ...
        if (scheduledTransferIds.size() > MAX_CANCEL_COUNT) {
            throw new BusinessException(
                    CommonErrorCode.INVALID_INPUT, "한 번에 취소할 수 있는 예약이체는 최대 " + MAX_CANCEL_COUNT + "건입니다.");
        }
    }
}
