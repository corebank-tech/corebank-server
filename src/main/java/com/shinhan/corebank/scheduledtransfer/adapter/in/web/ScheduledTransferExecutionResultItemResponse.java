...
public record ScheduledTransferExecutionResultItemResponse(
        ...
        @Schema(description = "처리결과 상태")
        ScheduledTransferStatus status,
        @Schema(description = "실행(확정) 시각. SUCCESS/FAILED만 채워짐", nullable = true)
        LocalDateTime executedAt,
        @Schema(description = "취소 시각. CANCELED만 채워짐", nullable = true)
        ...
        Long amount,
        @Schema(description = "거래번호. SUCCESS/FAILED만 채워짐", nullable = true)
        String transactionNumber,
        @Schema(description = "실패 사유. status=SUCCESS일 때는 비어있음", nullable = true)
