...
public class ScheduledTransferPersistenceAdapter
        implements ScheduledTransferPersistencePort, ScheduledTransferQueryPort, ScheduledTransferBatchQueryPort {
    ...
    public List<ScheduledTransfer> findAllProcessing() {
        return scheduledTransferJpaRepository.findByStatus(ScheduledTransferStatus.PROCESSING).stream()
                .map(ScheduledTransferMapper::toDomain)
                .toList();
    }
    ...
    public ScheduledTransfer save(ScheduledTransfer scheduledTransfer) {
        try {
            ScheduledTransferJpaEntity saved =
                    scheduledTransferJpaRepository.save(ScheduledTransferMapper.toEntity(scheduledTransfer));
    ...
    public Optional<ScheduledTransfer> findById(Long scheduledTransferId) {
        return scheduledTransferJpaRepository.findById(scheduledTransferId).map(ScheduledTransferMapper::toDomain);
    }
    ...
    @Override
    public Page<ScheduledTransfer> search(
            Long customerId,
        ...
        var query = queryFactory
                ...
                .where(conditions)
                .orderBy(
                        scheduledTransferJpaEntity.scheduledDate.asc(),
                        scheduledTransferJpaEntity.scheduledTransferId.asc());
        if (pageable.isPaged()) {
        ...
        Long total = queryFactory
                ...
                .where(conditions)
                .fetchOne();
        List<ScheduledTransfer> domainContent =
                content.stream().map(ScheduledTransferMapper::toDomain).toList();
        return new PageImpl<>(domainContent, pageable, total == null ? 0 : total);
    }

    @Override
    public Page<ScheduledTransfer> searchExecutionResults(
            Long customerId,
            Long withdrawalAccountId,
            LocalDate fromDate,
            LocalDate toDate,
    ...
    @Override
    // 정상/오류/취소 : 건수·금액 집계
    public ScheduledTransferExecutionResultAggregate summarizeExecutionResults(
            Long customerId, Long withdrawalAccountId, LocalDate fromDate, LocalDate toDate) {
        Predicate[] conditions = executionResultConditions(customerId, withdrawalAccountId, fromDate, toDate);
        List<Tuple> rows = queryFactory
                .select(
                        scheduledTransferJpaEntity.status,
                        scheduledTransferJpaEntity.count(),
        ...
        return new ScheduledTransferExecutionResultAggregate(
                successCount, successAmount, failedCount, failedAmount, canceledCount, canceledAmount);
    }
    ...
    private Predicate[] executionResultConditions(
            Long customerId, Long withdrawalAccountId, LocalDate fromDate, LocalDate toDate) {
        return new Predicate[] {
            scheduledTransferJpaEntity.customerId.eq(customerId),
            withdrawalAccountIdEq(withdrawalAccountId),
            ...
            scheduledTransferJpaEntity.status.in(
                    ScheduledTransferStatus.SUCCESS, ScheduledTransferStatus.FAILED, ScheduledTransferStatus.CANCELED)
        };
    }
    ...
    private ComparableExpression<LocalDateTime> effectiveDate() {
        return new Coalesce<>(
                        LocalDateTime.class,
                        scheduledTransferJpaEntity.executedAt,
                        scheduledTransferJpaEntity.canceledAt)
                .getValue();
    }
    ...
    private BooleanExpression withdrawalAccountIdEq(Long withdrawalAccountId) {
        return withdrawalAccountId != null
                ? scheduledTransferJpaEntity.withdrawalAccountId.eq(withdrawalAccountId)
                : null;
    }
