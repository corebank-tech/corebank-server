...
public record ScheduledTransferListItemResponse(
        ...
        @Schema(description = "출금계좌 ID") Long withdrawalAccountId,
        @Schema(description = "예약 실행일") LocalDate scheduledDate,
        ...
        @Schema(description = "입금은행명. 1차는 당행 전용이라 항상 \"신한은행\"", example = "신한은행") String payeeBankName,
        ...
        @Schema(description = "예약이체 상태") ScheduledTransferStatus status,
        @Schema(description = "취소 가능 여부 (상태가 WAITING이고 실행 예정일이 오늘 이후면 true)") boolean cancelable,
