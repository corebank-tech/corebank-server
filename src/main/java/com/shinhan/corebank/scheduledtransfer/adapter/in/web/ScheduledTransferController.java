...
public class ScheduledTransferController {
    ...
    @Operation(
            operationId = "registerScheduledTransfer",
            summary = "예약이체 등록",
            description =
                    """
    ...
    @PostMapping("/cancel")
    @Operation(
            operationId = "cancelScheduledTransfers",
            summary = "예약이체 취소",
            description =
                    """
    ...
    public ResponseEntity<ApiResponse<ScheduledTransferCancelResponse>> cancel(
        ...
        return withIdempotency(
                ...
                new TypeReference<>() {},
                () -> ApiResponse.success(
                        ScheduledTransferCancelResponse.from(scheduledTransferCancelUseCase.cancel(command))));
    }
    ...
    @Operation(
            ...
            summary = "예약이체 목록조회",
            description = "등록된 예약이체(대기중·실행완료·취소 등) 목록을 상태·출금계좌·조회기간으로 조회한다.")
    @ApiResponses({
    ...
    public ApiResponse<PageResponse<ScheduledTransferListItemResponse>> search(
            @Parameter(description = "예약이체 상태 필터. 미전달 또는 ALL이면 전체 조회", example = "WAITING")
                    ...
                    String status,
            @Parameter(description = "출금계좌 ID (내 계좌). 미전달 시 전체 계좌 대상", example = "1001") @RequestParam(required = false)
    ...
    @Operation(
            operationId = "getScheduledTransferExecutions",
