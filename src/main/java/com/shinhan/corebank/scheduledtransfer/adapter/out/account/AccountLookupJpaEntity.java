...
// account 테이블 겨냥한 scheduledtransfer 도메인 전용 경량 매핑(조회 전용). account 패키지를 import하지 않는다.
// 엔티티명 명시: autotransfer.adapter.out.account.AccountLookupJpaEntity와 클래스 단순이름이 같아
// 기본 JPA 엔티티명(simple class name)이 충돌한다(Hibernate DuplicateMappingException).
@Entity(name = "ScheduledTransferAccountLookupJpaEntity")
@Table(name = "account")
@Getter
