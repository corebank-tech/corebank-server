...
public class AutoTransferBatchItemProcessor {
    ...
    public void completeProcessing(
            AutoTransfer autoTransfer, AutoTransferExecution processingExecution, LocalDate date) {
        ...
        if (result.status() == ProcessResultStatus.SUCCESS) {
            ...
            log.warn(
                    "자동이체 실행 실패 - autoTransferId={}, date={}, errorCode={}, errorMessage={}",
        ...
        if (StringUtils.hasText(processingExecution.getTransactionNumber())) {
            ...
            auditLogService.record(
                    autoTransfer.getCustomerId(),
                    processingExecution.getTransactionNumber(),
    ...
    public void reconcileStuckExecution(StuckExecution stuckExecution) {
        ...
        if (lookup.isEmpty()) {
            ...
            log.warn(
                    "재확정 배치가 예상치 못한 transfer 상태를 조회함 - autoTransferId={}, status={}",
                    autoTransfer.getAutoTransferId(),
                    lookup.get().status());
            execution.markError("실행 중 확인 불가로 재확정 배치가 오류 처리함", null);
        }
        ...
        if (!confirmed) {
            log.info(
                    "이미 다른 재확정 실행이 먼저 확정함(중복 재확정 방어) - autoTransferId={}, executionId={}",
                    autoTransfer.getAutoTransferId(),
        ...
        if (execution.getStatus() == ProcessResultStatus.ERROR) {
            ...
            if (allFailed) {
                log.warn(
                        "자동이체 연속 실패 감지 - autoTransferId={}, 연속실패건수={}",
                        autoTransfer.getAutoTransferId(),
