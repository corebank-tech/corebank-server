    ...
    public List<AutoTransferCancelResult> cancel(AutoTransferCancelCommand command) {
        ...
        for (Long autoTransferId : command.autoTransferIds()) {
            ...
            if (found.isEmpty() || !found.get().getCustomerId().equals(command.customerId())) {
                results.put(autoTransferId, AutoTransferCancelResult.failure(autoTransferId, AutoTransferErrorCode.NOT_FOUND));
                continue;
            }
            ...
            if (!autoTransfer.getStatus().isModifiable()) {
                results.put(autoTransferId, AutoTransferCancelResult.failure(
                        autoTransferId, AutoTransferErrorCode.NOT_IN_NORMAL_STATUS));
                continue;
            }
