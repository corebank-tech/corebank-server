...
@Builder
public record AutoTransferCancelCommand(
        Long customerId,
    ...
    public AutoTransferCancelCommand {
        ...
        if (autoTransferIds == null
                || autoTransferIds.isEmpty()
                || autoTransferIds.stream().anyMatch(Objects::isNull)) {
            throw new BusinessException(CommonErrorCode.REQUIRED_FIELD_MISSING);
        }
        ...
        if (autoTransferIds.size() > MAX_CANCEL_COUNT) {
            throw new BusinessException(
                    CommonErrorCode.INVALID_INPUT, "한 번에 해지할 수 있는 자동이체는 최대 " + MAX_CANCEL_COUNT + "건입니다.");
        }
    }
}
