...
public interface AutoTransferExecutionJpaRepository extends JpaRepository<AutoTransferExecutionJpaEntity, Long> {
    ...
    @Modifying
    @Query(value = "UPDATE auto_transfer_execution SET status = :status, transaction_number = :transactionNumber, "
            + "failure_reason = :failureReason "
            + "WHERE execution_id = :executionId AND status = 'PROCESSING'",
            nativeQuery = true)
    int finalizeIfProcessing(@Param("executionId") Long executionId, @Param("status") String status,
