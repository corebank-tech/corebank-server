...
public record AutoTransferListItemResponse(
        ...
        @Schema(description = "출금계좌 별칭 (미설정 시 null)") String fromAlias,
        @Schema(description = "예금주명") String payeeName,
        @Schema(description = "회당 이체금액") Long amount,
        @Schema(description = "이체 시작일") LocalDate startDate,
        @Schema(description = "이체 종료일") LocalDate endDate,
        @Schema(description = "이체지정일") Integer transferDay,
        @Schema(description = "이체주기(개월)") Integer cycleMonths,
        @Schema(description = "내 통장 표시내용") String myPassbookMemo,
        @Schema(description = "자동이체 상태") AutoTransferStatus status,
        @Schema(description = "해지 가능 여부 (상태가 NORMAL이고 다음 실행 예정일이 오늘이 아니면 true)") boolean cancelable,
        @Schema(description = "등록일시") LocalDateTime registeredAt) {
    public static AutoTransferListItemResponse from(AutoTransferListItem item) {
        return new AutoTransferListItemResponse(
                item.autoTransferId(),
