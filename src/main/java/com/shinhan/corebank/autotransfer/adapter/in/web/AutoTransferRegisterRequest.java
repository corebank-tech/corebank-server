...
public record AutoTransferRegisterRequest(
        ...
        @Schema(description = "입금계좌번호 (하이픈 없이)", example = "110123456789") String depositAccountNumber,
        @Schema(description = "예금주명", example = "홍길동") String payeeName,
        @Schema(description = "회당 이체금액. 1원 이상의 정수", example = "50000", minimum = "1") Long amount,
        @Schema(description = "이체주기(개월). 1/3/6 중 하나", example = "1") Integer cycleMonths,
        @Schema(description = "매 이체주기 내 이체지정일. 1~31", example = "25", minimum = "1", maximum = "31")
                Integer transferDay,
        @Schema(description = "이체 시작일. 익일부터 1년 이내", example = "2026-09-01") LocalDate startDate,
