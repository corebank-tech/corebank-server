...
public record AutoTransferExecutionHistoryPageResponse(
        @Schema(description = "조회 기간 내 성공/실패 건수·금액 집계") AutoTransferExecutionHistorySummaryResponse summary,
        ...
        @Schema(description = "실행 이력 목록") List<AutoTransferExecutionHistoryItemResponse> items) {
    public static AutoTransferExecutionHistoryPageResponse from(AutoTransferExecutionHistoryResult result) {
        ...
        return new AutoTransferExecutionHistoryPageResponse(
                ...
                page.getContent().stream()
                        .map(AutoTransferExecutionHistoryItemResponse::from)
                        .toList());
    }
}
