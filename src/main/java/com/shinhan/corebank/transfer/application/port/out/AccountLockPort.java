...
public interface AccountLockPort {
    ...
    /**
     * 출금계좌 잔액·상태 사전 체크(락 없음)용 조회. {@link WithdrawalAccountPreCheckSnapshot}
     * 참고 — OTP 소비 전 낭비 방지용이며 최종 판정이 아니다.
     */
    Optional<WithdrawalAccountPreCheckSnapshot> findWithdrawalAccountPreCheckSnapshot(Long accountId);
     ...
     * 적용하고, 변경 후 잔액을 반환한다. 두 계좌의 last_transaction_at도 executedAt으로
     * 함께 갱신한다(#150).
     ...
     */
    TransferBalances applyTransfer(LockedAccountsForTransfer locked, long amount, LocalDateTime executedAt);
}
