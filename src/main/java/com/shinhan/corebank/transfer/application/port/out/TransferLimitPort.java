 ...
 *
 * <p>1회/1일 한도 위반 시 이 메서드 스스로 BusinessException(LmtErrorCode)을 던진다 —
 * 호출자가 반환값을 비교해 예외를 던지는 게 아니다. 1일 누적 사용량을 같은 트랜잭션 안에서
 * 원자적으로 검사+적립(reserve)해야 하므로, 그 원자적 동작을 포트 구현체 내부로 캡슐화해 둔다.
 */
