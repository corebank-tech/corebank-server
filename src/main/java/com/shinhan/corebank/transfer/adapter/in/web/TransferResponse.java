...
public record TransferResponse(
        ...
        @Schema(description = "이체 처리 시각. status=SUCCESS일 때만 채워짐") LocalDateTime transferredAt,
        @Schema(description = "실패 코드. status=ERROR일 때만 채워짐 (예: TRF0303)", nullable = true) String errorCode,
        @Schema(description = "실패 메시지. status=ERROR일 때만 채워짐", nullable = true) String errorMessage,
        @Schema(description = "이체 후 출금계좌 잔액. status=SUCCESS일 때만 채워짐") Long withdrawalBalanceAfter) {
