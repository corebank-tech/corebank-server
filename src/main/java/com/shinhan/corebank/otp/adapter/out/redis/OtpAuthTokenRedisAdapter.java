...
public class OtpAuthTokenRedisAdapter implements OtpAuthTokenStorePort {
    ...
    public OtpAuthTokenRedisAdapter(StringRedisTemplate redisTemplate, ObjectMapper objectMapper) {
    ...
    public boolean consumeIfMatches(String otpAuthToken, OtpAuthTokenPayload expectedPayload) {
        Long result = redisTemplate.execute(CONSUME_SCRIPT, List.of(key(otpAuthToken)), serialize(expectedPayload));
        if (result == null) {
