...
public class LoginService implements LoginUseCase {
    ...
    public LoginResult login(LoginCommand command) {
        ...
        boolean passwordMatches = passwordHashVerifierPort.matches(
                command.password(),
                loginCustomer.getPasswordHash()
        );
    ...
    private LoginFailedException handlePasswordMismatch(
            LoginCustomer loginCustomer,
            String requestIp
    ) {
        LoginFailureUpdateResult updateResult =
                loginCustomerPort.recordLoginFailure(
                        loginCustomer.getCustomerId()
                );
        ...
        LoginAttemptResult attemptResult = loginAttemptProcessor.process(
    ...
    private void recordFailureAudit(
        ...
        try {
            ...
            log.error(
                    "로그인 실패 감사 로그 저장에 실패했습니다. customerId={}, reason={}",
                    customerId,
                    reason,
                    exception
            );
