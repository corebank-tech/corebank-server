...
public class FindIdService implements FindIdUseCase {
    ...
    private static final Pattern ACCOUNT_PASSWORD_PATTERN = Pattern.compile("^\\d{4}$");
    ...
    public FindIdResult findId(FindIdCommand command) {
        ...
        List<FindIdCustomerCandidate> candidates = customerPort.findAllByIdentity(command.customerName(), birthDate);
        if (candidates.isEmpty()) {
            throw new BusinessException(AuthErrorCode.USER_NOT_FOUND);
        }

        Set<Long> candidateIds =
                candidates.stream().map(FindIdCustomerCandidate::customerId).collect(Collectors.toUnmodifiableSet());
        FindIdAccountVerificationResult verification =
                accountVerificationPort.verify(candidateIds, command.accountNumber(), command.accountPassword());

        return switch (verification.status()) {
            case INFORMATION_MISMATCH, PASSWORD_MISMATCH ->
                throw new BusinessException(AuthErrorCode.IDENTITY_INFORMATION_MISMATCH);
            case LOCKED -> throw new BusinessException(AuthErrorCode.ACCOUNT_LOCKED);
            case VERIFIED -> new FindIdResult(findUserId(candidates, verification.customerId()));
        };
    }
    ...
    private LocalDate parseAndValidate(FindIdCommand command) {
        if (command.customerName().length() > 50
                || !ACCOUNT_NUMBER_PATTERN.matcher(command.accountNumber()).matches()
                || !ACCOUNT_PASSWORD_PATTERN.matcher(command.accountPassword()).matches()) {
    ...
    private String findUserId(List<FindIdCustomerCandidate> candidates, Long verifiedCustomerId) {
        return candidates.stream()
                .filter(candidate -> candidate.customerId().equals(verifiedCustomerId))
                .map(FindIdCustomerCandidate::userId)
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("계좌 검증 고객이 아이디 찾기 후보에 없습니다."));
    }
