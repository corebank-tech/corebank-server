...
public class FindIdCustomerAdapter implements FindIdCustomerPort {
    ...
    @Override
    public List<FindIdCustomerCandidate> findAllByIdentity(String customerName, LocalDate birthDate) {
        return customerFacade.findAllByIdentity(customerName, birthDate).stream()
                .map(customer -> new FindIdCustomerCandidate(customer.customerId(), customer.userId()))
                .toList();
    }
}
