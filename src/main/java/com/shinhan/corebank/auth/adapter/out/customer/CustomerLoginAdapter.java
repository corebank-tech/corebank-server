...
public class CustomerLoginAdapter implements LoginCustomerPort {
    ...
    public Optional<LoginCustomer> findByUserId(String userId) {
        ...
        return customerAuthenticationFacade.findByUserId(userId).map(customerAuthenticationMapper::toLoginCustomer);
    }
    ...
    public LoginFailureUpdateResult recordLoginFailure(Long customerId) {
        ...
        LoginFailureState state =
                customerAuthenticationFacade.updateLoginFailureState(new RecordLoginFailureCommand(customerId));

        return new LoginFailureUpdateResult(state.loginFailureCount(), state.accountLocked());
    }

    // 로그인 성공 결과와 접속정보를 customer 모듈에 전달
    @Override
    public LoginSuccessUpdateResult recordLoginSuccess(Long customerId, LocalDateTime loginAt, String loginIp) {
        Objects.requireNonNull(customerId, "customerId must not be null");
        Objects.requireNonNull(loginAt, "loginAt must not be null");
        Objects.requireNonNull(loginIp, "loginIp must not be null");

        RecordLoginSuccessCommand command = new RecordLoginSuccessCommand(customerId, loginAt, loginIp);

        LoginSuccessState state = customerAuthenticationFacade.updateLoginSuccessState(command);
