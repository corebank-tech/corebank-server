...
public class CustomerPersistenceAdapter implements CustomerPersistencePort {
    ...
    public Optional<Customer> findByIdForUpdate(Long customerId) {
        ...
        return customerJpaRepository.findByIdForUpdate(customerId).map(customerMapper::toDomain);
    }
    ...
    public boolean existsByExistingBankCustomerId(String existingBankCustomerId) {
        Objects.requireNonNull(existingBankCustomerId, "existingBankCustomerId must not be null");
        return customerJpaRepository.existsByExistingBankCustomerId(existingBankCustomerId);
    }
    ...
    public void updateLoginFailureState(Customer customer) {
        ...
        entity.updateLoginFailureState(customer.getLoginFailureCount(), customer.isAccountLocked());
    ...
    public Customer updateContactInfo(Customer customer) {
        ...
        CustomerJpaEntity entity = findExistingEntityForUpdate(customer.getCustomerId());
        entity.updateContactInfo(customer.getPhoneNumber(), customer.getEmail());
    ...
    public Customer save(Customer customer) {
        ...
        if (customer.getCustomerId() != null) {
            throw new IllegalArgumentException("신규 고객은 customerId가 없어야 합니다.");
        }

        CustomerJpaEntity newEntity = customerMapper.toEntity(customer);

        CustomerJpaEntity savedEntity = customerJpaRepository.save(newEntity);
    ...
    private CustomerJpaEntity findExistingEntityForUpdate(Long customerId) {
        ...
        return customerJpaRepository
                ...
                .orElseThrow(() -> new IllegalStateException("저장할 고객이 존재하지 않습니다."));
    }
}
