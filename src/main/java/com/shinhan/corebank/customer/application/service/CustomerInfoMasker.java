...
public class CustomerInfoMasker {
    ...
    public String maskUserId(String userId) {
        if (userId == null || userId.length() < 5) {
            throw new IllegalArgumentException("마스킹할 로그인 아이디 형식이 올바르지 않습니다.");
        }
    ...
    public String maskBirthDate(LocalDate birthDate) {
        if (birthDate == null) {
            throw new IllegalArgumentException("마스킹할 생년월일이 필요합니다.");
        }
    ...
    public String maskPhoneNumber(String phoneNumber) {
        if (phoneNumber == null || !PHONE_PATTERN.matcher(phoneNumber).matches()) {
            throw new IllegalArgumentException("마스킹할 휴대폰번호 형식이 올바르지 않습니다.");
        }
    ...
    public String maskEmail(String email) {
        if (email == null) {
            throw new IllegalArgumentException("마스킹할 이메일이 필요합니다.");
        }

        int atIndex = email.indexOf('@');
        if (atIndex <= 0 || atIndex == email.length() - 1) {
            throw new IllegalArgumentException("마스킹할 이메일 형식이 올바르지 않습니다.");
        }

        String localPart = email.substring(0, atIndex);
        String domainPart = email.substring(atIndex);
        int visibleLength = localPart.length() <= 4 ? Math.max(0, localPart.length() - 1) : 4;
        int maskedLength = localPart.length() - visibleLength;

        return localPart.substring(0, visibleLength) + "*".repeat(maskedLength) + domainPart;
    }
}
