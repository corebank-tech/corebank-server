...
public class CustomerAuthenticationService implements CustomerAuthenticationFacade {
    ...
    @Override
    @Transactional
    public LoginFailureState updateLoginFailureState(RecordLoginFailureCommand command) {
        ...
        Customer customer = customerPersistencePort
                ...
                .orElseThrow(() -> new IllegalStateException("로그인 상태를 변경할 고객이 존재하지 않습니다."));

        // 다른 요청이 먼저 잠근 계정이면 상태를 변경하지 않고 최신 상태를 반환
        if (customer.isAccountLocked()) {
            return new LoginFailureState(customer.getLoginFailureCount(), true);
        }
        ...
        return new LoginFailureState(customer.getLoginFailureCount(), customer.isAccountLocked());
    }

    // 로그인 성공 시 실패 횟수를 초기화하고 최근 접속정보 저장
    @Override
    @Transactional
    public LoginSuccessState updateLoginSuccessState(RecordLoginSuccessCommand command) {
        Objects.requireNonNull(command, "command must not be null");
        Objects.requireNonNull(command.customerId(), "customerId must not be null");
