...
public class AccountTransactionQueryService implements AccountTransactionQueryUseCase {
    ...
    public LedgerHistoryResult getTransactions(
        ...
        validateOwnership(customerId, accountId);

        LocalDate resolvedToDate = toDate != null ? toDate : LocalDate.now(clock.withZone(KOREA_ZONE));

        LocalDate resolvedFromDate = fromDate != null ? fromDate : resolvedToDate.minusMonths(1);

        LedgerHistoryDirection resolvedDirection = direction != null ? direction : LedgerHistoryDirection.ALL;

        LedgerHistorySort resolvedSort = sort != null ? sort : LedgerHistorySort.LATEST;

        validateDateRange(resolvedFromDate, resolvedToDate);

        validatePagination(page, size, all);
    ...
    private void validateRequired(Long customerId, Long accountId) {
        if (customerId == null || accountId == null) {
            throw new BusinessException(CommonErrorCode.REQUIRED_FIELD_MISSING);
        }
    }

    private void validateOwnership(Long customerId, Long accountId) {
    ...
    private void validatePagination(int page, int size, boolean all) {
        if (all) {
            return;
        }
