...
public class AccountPasswordVerificationService
        implements VerifyAccountPasswordUseCase {
    ...
    @Override
    public VerifyAccountPasswordResult verify(
            VerifyAccountPasswordCommand command
    ) {
        ...
        if (attempt.locked()) {
            throw new AccountPasswordVerificationFailedException(
        ...
        if (!attempt.matched()) {
            throw new AccountPasswordVerificationFailedException(
                    AccountPasswordErrorCode.PASSWORD_MISMATCH,
                    attempt
            );
        }
        ...
        return new VerifyAccountPasswordResult(
                command.accountId(),
                true,
                token,
                0,
