...
public class AccountIdentityVerificationService
    ...
    public AccountIdentityVerificationResult verify(
        ...
        if (account == null
                || !verification.candidateCustomerIds()
                        .contains(account.getCustomerId())) {
            return result(
        ...
        AccountPasswordAttemptResult attempt = passwordProcessor.verify(
                ...
                verification.accountPassword()
        );

        if (attempt.locked()) {
            return result(
                    AccountIdentityVerificationStatus.LOCKED,
                    account.getCustomerId()
            );
        }
        if (!attempt.matched()) {
        ...
        return result(
                AccountIdentityVerificationStatus.VERIFIED,
                account.getCustomerId()
        );
    }

    private AccountIdentityVerificationResult result(
