...
@Transactional
public class AccountPasswordChangeService implements ChangeAccountPasswordUseCase {

    private static final Pattern PASSWORD_PATTERN = Pattern.compile("^\\d{4}$");
    private static final ZoneId KOREA_ZONE = ZoneId.of("Asia/Seoul");

    private final AccountPersistencePort accountPersistencePort;
    private final AccountPasswordChangeAuthVerificationPort authVerificationPort;
    private final PasswordEncoder passwordEncoder;
    private final AuditLogService auditLogService;

    @Override
    public ChangeAccountPasswordResult change(ChangeAccountPasswordCommand command) {
        ...
        authVerificationPort.verifyOtpToken(command.otpAuthToken(), command.customerId(), command.accountId());

        String encodedPassword = passwordEncoder.encode(command.newAccountPassword());

        Account lockedAccount = accountPersistencePort
                .findByAccountIdAndCustomerIdForUpdate(command.accountId(), command.customerId())
                .orElseThrow(() -> new BusinessException(AccountErrorCode.ACCOUNT_NOT_FOUND_OR_FORBIDDEN));
        ...
        Account saved = accountPersistencePort.updatePasswordState(lockedAccount);
        ...
        return new ChangeAccountPasswordResult(
                saved.getAccountId(), saved.getUpdatedAt().atZone(KOREA_ZONE).toOffsetDateTime());
    }

    // 필수값과 신규 비밀번호 숫자 4자리 및 확인값 일치를 검증한다.
    private void validateCommand(ChangeAccountPasswordCommand command) {
        ...
        if (command.customerId() <= 0
                || command.accountId() <= 0
                || !PASSWORD_PATTERN.matcher(command.newAccountPassword()).matches()
                || !PASSWORD_PATTERN
                        .matcher(command.newAccountPasswordConfirm())
                        .matches()) {
