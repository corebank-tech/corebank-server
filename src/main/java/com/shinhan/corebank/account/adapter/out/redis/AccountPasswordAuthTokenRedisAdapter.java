...
public class AccountPasswordAuthTokenRedisAdapter implements AccountPasswordAuthTokenStorePort {
    ...
    public AccountPasswordAuthTokenRedisAdapter(StringRedisTemplate redisTemplate, ObjectMapper objectMapper) {
    ...
    public boolean consumeIfMatches(String token, AccountPasswordAuthTokenPayload expectedPayload) {
        Long result = redisTemplate.execute(CONSUME_SCRIPT, List.of(key(token)), serialize(expectedPayload));
    ...
    private String serialize(AccountPasswordAuthTokenPayload payload) {
        try {
            return objectMapper.writeValueAsString(payload);
        } catch (JacksonException exception) {
            throw new IllegalStateException("계좌비밀번호 인증 토큰 직렬화에 실패했습니다.", exception);
        }
    }
