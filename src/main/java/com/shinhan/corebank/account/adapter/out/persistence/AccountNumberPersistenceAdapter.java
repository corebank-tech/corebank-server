...
public class AccountNumberPersistenceAdapter implements AccountNumberSequencePort {
    ...
    public Optional<AccountNumberSequence> findForUpdate(String bankCode, AccountType accountType, Long productId) {
        ...
        if (productId == null) {
            entity = repository.findDemandDepositForUpdate(bankCode, accountType);
        } else {
            entity = repository.findProductAccountForUpdate(bankCode, accountType, productId);
        }
    ...
    public void update(AccountNumberSequence accountNumberSequence) {
        AccountNumberSequenceJpaEntity entity = repository
                .findById(accountNumberSequence.getSequenceId())
                .orElseThrow(() -> new IllegalStateException("잠금 조회한 채번 행이 존재하지 않습니다."));
        entity.updateLastSequence(accountNumberSequence.getLastSequence());
    }
}
