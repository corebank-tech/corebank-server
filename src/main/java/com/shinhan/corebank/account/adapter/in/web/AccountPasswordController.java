...
@Validated
@Tag(name = "계좌", description = "계좌 조회 및 계좌비밀번호 인증 API")
public class AccountPasswordController {

    private static final String SUCCESS_MESSAGE = "계좌비밀번호 검증이 완료되었습니다.";
    private static final String CHANGE_SUCCESS_MESSAGE = "계좌비밀번호가 변경되었습니다.";
    ...
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "계좌비밀번호 검증 성공"),
    ...
    public ApiResponse<AccountPasswordVerifyResponse> verify(
            @Parameter(description = "계좌비밀번호를 검증할 계좌의 내부 식별자", required = true, example = "101") @PathVariable @Positive
                    Long accountId,
            @RequestBody AccountPasswordVerifyRequest request) {
        Long customerId = currentCustomerProvider.getCurrentCustomerId();

        VerifyAccountPasswordResult result = verifyUseCase.verify(
                new VerifyAccountPasswordCommand(customerId, accountId, request.accountPassword()));

        return ApiResponse.success(AccountPasswordVerifyResponse.from(result), SUCCESS_MESSAGE);
    }
    ...
    @Operation(
            ...
            description =
                    ...
                    """)
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "계좌비밀번호 변경 성공"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "400",
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "403",
                description =
                        "`APW0101` 비밀번호 오류 잠금 · `APW0102` 계좌비밀번호 인증 토큰 오류 · `OTP0101` OTP 인증 토큰 오류 · `OTP0102` OTP 거래정보 불일치",
                content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "404",
                description = "`ACC0201` 계좌를 찾을 수 없거나 접근할 수 없음",
    ...
    public ResponseEntity<ApiResponse<AccountPasswordChangeResponse>> change(
            @Parameter(description = "비밀번호를 변경할 계좌의 내부 식별자", required = true, example = "101") @PathVariable @Positive
                    Long accountId,
        ...
        Long customerId = currentCustomerProvider.getCurrentCustomerId();
        String endpoint = "PUT /accounts/" + accountId + "/password";
        ...
        return idempotentRequestExecutor.execute(
                ...
                new TypeReference<>() {},
                () -> {
                    ChangeAccountPasswordResult result = changeUseCase.change(new ChangeAccountPasswordCommand(
                            customerId,
                            accountId,
                    ...
                    return ApiResponse.success(AccountPasswordChangeResponse.from(result), CHANGE_SUCCESS_MESSAGE);
                });
    }

    // 일회성 인증 토큰은 제외하고 실제 변경 대상과 신규 비밀번호만 멱등 지문에 포함한다.
    private Map<String, Object> changeFingerprint(
            Long customerId, Long accountId, AccountPasswordChangeRequest request) {
