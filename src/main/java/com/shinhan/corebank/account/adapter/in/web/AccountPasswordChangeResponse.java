...
public record AccountPasswordChangeResponse(
        ...
        @Schema(description = "계좌비밀번호 변경 일시", example = "2026-08-22T13:30:00+09:00") OffsetDateTime updatedAt) {

    // application 결과를 계좌비밀번호 변경 API 응답으로 변환한다.
    public static AccountPasswordChangeResponse from(ChangeAccountPasswordResult result) {
        return new AccountPasswordChangeResponse(result.accountId(), result.updatedAt());
    }
}
