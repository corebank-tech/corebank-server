...
public record AccountDetailResponse(
        @Schema(
                description = "조회 기준 일시",
                example = "2026-07-30T15:50:00+09:00"
        )
        OffsetDateTime asOf,
        ...
        @Schema(description = "화면에 표시할 계좌명", example = "생활비 통장")
        String accountName,

        @Schema(description = "일부 마스킹한 예금주명", example = "홍*동")
        String ownerName,

        @Schema(description = "하이픈 없는 계좌번호", example = "110550051877")
        String accountNumber,

        @Schema(description = "현재 잔액(원)", example = "1500000")
        long balance,

        @Schema(description = "출금 가능 잔액(원). 출금 제한 시 0", example = "1500000")
        long availableBalance,

        @Schema(description = "계좌 개설일", example = "2025-03-10")
        LocalDate openedDate,

        @Schema(description = "계좌 상태", example = "ACTIVE")
        AccountStatus status,

        @Schema(description = "계좌비밀번호 연속 오류 횟수", example = "2")
        int passwordFailureCount,

        @Schema(description = "계좌비밀번호 잠금 여부", example = "false")
