...
@Validated
@Tag(name = "출금계좌", description = "출금계좌 관리 API")
public class WithdrawalAccountController {

    private static final String REGISTER_SUCCESS_MESSAGE = "출금계좌가 등록되었습니다.";

    private static final String UNREGISTER_SUCCESS_MESSAGE = "출금계좌 등록이 삭제되었습니다.";

    private final WithdrawalAccountRegisterUseCase withdrawalAccountRegisterUseCase;

    private final WithdrawalAccountUnregisterUseCase withdrawalAccountUnregisterUseCase;
    ...
    @Operation(
            ...
            description =
                    ...
                    """)
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "출금계좌 등록 성공"),
    ...
    public ResponseEntity<ApiResponse<WithdrawalAccountRegisterResponse>> register(
            @Parameter(description = "출금계좌로 등록할 계좌의 내부 식별자", required = true, example = "101") @PathVariable @Positive
                    Long accountId,
            @Parameter(
            ...
            @Valid @RequestBody WithdrawalAccountRegisterRequest request) {
        Long customerId = currentCustomerProvider.getCurrentCustomerId();

        String endpoint = "PUT /withdrawal-accounts/" + accountId;

        return idempotentRequestExecutor.execute(
                idempotencyKey,
                customerId,
                endpoint,
                fingerprint(customerId, accountId),
                new TypeReference<>() {},
                () -> {
                    WithdrawalAccountRegisterResult result =
                            withdrawalAccountRegisterUseCase.register(request.toCommand(customerId, accountId));

                    return ApiResponse.success(
                            WithdrawalAccountRegisterResponse.from(result), REGISTER_SUCCESS_MESSAGE);
                });
    }
    ...
    @Operation(
            ...
            description =
                    ...
                    """)
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "출금계좌 등록 삭제 성공"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "400",
    ...
    public ResponseEntity<ApiResponse<WithdrawalAccountUnregisterResponse>> unregister(
        ...
        String endpoint = "DELETE /withdrawal-accounts/" + accountId;
        ...
        return idempotentRequestExecutor.execute(
                ...
                new TypeReference<>() {},
                () -> {
                    WithdrawalAccountUnregisterResult result = withdrawalAccountUnregisterUseCase.unregister(
                            new WithdrawalAccountUnregisterCommand(customerId, accountId));

                    return ApiResponse.success(
                            WithdrawalAccountUnregisterResponse.from(result), UNREGISTER_SUCCESS_MESSAGE);
                });
    }

    private Map<String, Object> fingerprint(Long customerId, Long accountId) {
