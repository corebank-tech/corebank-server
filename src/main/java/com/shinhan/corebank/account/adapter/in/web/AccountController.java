...
@Validated
@Tag(name = "계좌", description = "계좌 조회 및 계좌별명 관리 API")
public class AccountController {
    ...
    @GetMapping
    @Operation(operationId = "getAccounts", summary = "전체 계좌 조회", description = "로그인한 고객이 보유한 전체 계좌를 유형별로 그룹화하여 조회한다.")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공")
    public ApiResponse<AccountOverviewResponse> getAccounts() {
        Long customerId = currentCustomerProvider.getCurrentCustomerId();

        AccountOverviewResult result = accountOverviewQueryUseCase.getOverview(customerId);

        return ApiResponse.success(AccountOverviewResponse.from(result));
    }
    ...
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "계좌 상세 조회 성공"),
    ...
    public ApiResponse<AccountDetailResponse> getAccountDetail(
        ...
        AccountDetailResult result = accountDetailQueryUseCase.getDetail(customer.customerId(), accountId);

        return ApiResponse.success(AccountDetailResponse.from(result, customer.userName()));
    }
    ...
    @Operation(
            ...
            description =
                    ...
                    """)
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "계좌별명 등록/수정 성공"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "400",
    ...
    public ResponseEntity<ApiResponse<AccountAliasResponse>> changeAlias(
            @Parameter(description = "별명을 등록하거나 수정할 계좌의 내부 식별자", required = true, example = "101")
                    @PathVariable
                    @Positive
                    Long accountId,
            @Parameter(
            ...
            @RequestBody AccountAliasChangeRequest request) {
        Long customerId = currentCustomerProvider.getCurrentCustomerId();

        String endpoint = "PUT /accounts/" + accountId + "/alias";
        ...
        return idempotentRequestExecutor.execute(
                ...
                new TypeReference<>() {},
                () -> {
                    AccountAliasResult result = accountAliasUseCase.changeAlias(customerId, accountId, request.alias());

                    return ApiResponse.success(AccountAliasResponse.from(result));
                });
    }
    ...
    @Operation(
            ...
            description =
                    ...
                    """)
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "계좌별명 삭제 성공"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
                responseCode = "400",
    ...
    public ResponseEntity<ApiResponse<Void>> deleteAlias(
            @Parameter(description = "별명을 삭제할 계좌의 내부 식별자", required = true, example = "101") @PathVariable @Positive
                    Long accountId,
            @Parameter(
                            description = "멱등키. 동일 키와 동일 요청 재요청 시 저장된 응답 반환",
                            required = true,
        ...
        String endpoint = "DELETE /accounts/" + accountId + "/alias";

        return idempotentRequestExecutor.execute(
                idempotencyKey,
                customerId,
                endpoint,
                deleteFingerprint(customerId, accountId),
                new TypeReference<>() {},
                () -> {
                    accountAliasUseCase.deleteAlias(customerId, accountId);
    ...
    public ApiResponse<AccountTransactionResponse> getTransactions(
        ...
        LedgerHistoryResult result = accountTransactionQueryUseCase.getTransactions(
                customerId, accountId, fromDate, toDate, direction, keyword, sort, page, size, all);

        return ApiResponse.success(AccountTransactionResponse.from(result));
    }

    private Map<String, Object> changeFingerprint(Long customerId, Long accountId, AccountAliasChangeRequest request) {
    ...
    private Map<String, Object> deleteFingerprint(Long customerId, Long accountId) {
        Map<String, Object> fingerprint = new LinkedHashMap<>();
