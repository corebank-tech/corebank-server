...
public class AccountNumberSequence {
    ...
    public static AccountNumberSequence reconstitute(
            ...
            String productPrefix,
            long lastSequence) {
        validateState(sequenceId, bankCode, accountType, productId, productPrefix, lastSequence);

        return new AccountNumberSequence(sequenceId, bankCode, accountType, productId, productPrefix, lastSequence);
    }
    ...
    public String issueNext() {
        if (isExhausted()) {
            throw new IllegalStateException("계좌번호 일련번호가 소진되었습니다.");
        }
    ...
    private static void validateState(
        ...
        if (sequenceId == null || sequenceId <= 0) {
            throw new IllegalStateException("채번 식별자가 올바르지 않습니다.");
        }
        if (bankCode == null || !bankCode.matches("^[0-9]{3}$")) {
            throw new IllegalStateException("사전 정의된 은행코드가 숫자 3자리가 아닙니다.");
        }
        if (accountType == null) {
            throw new IllegalStateException("사전 정의된 계좌 유형이 없습니다.");
        }
        if (productPrefix == null || !productPrefix.matches("^[0-9]{2}$")) {
            throw new IllegalStateException("사전 정의된 상품 prefix가 숫자 2자리가 아닙니다.");
        }
        if (lastSequence < 0 || lastSequence > AccountNumberPolicy.MAX_SEQUENCE) {
            throw new IllegalStateException("계좌번호 일련번호가 올바르지 않습니다.");
        }
        ...
        if (invalidProductCombination) {
            throw new IllegalStateException("계좌 유형과 상품 ID 조합이 올바르지 않습니다.");
        }
    }
    ...
    private static void validateGeneratedAccountNumber(String accountNumber) {
        if (accountNumber.length() != AccountNumberPolicy.ACCOUNT_NUMBER_LENGTH) {
            throw new IllegalStateException("생성된 계좌번호 길이가 올바르지 않습니다.");
        }
        if (!ACCOUNT_NUMBER_PATTERN.matcher(accountNumber).matches()) {
            throw new IllegalStateException("생성된 계좌번호가 숫자 12자리가 아닙니다.");
        }
    }
}
