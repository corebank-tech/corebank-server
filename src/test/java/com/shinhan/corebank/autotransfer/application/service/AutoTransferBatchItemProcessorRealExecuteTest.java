 ...
 * TransferExecutionUseCase를 @MockitoBean으로 대체하지 않고 real TransferExecutionService를
 * 그대로 주입받아 completeProcessing()을 돌려서 실제로 안전한지 확인한다.
 *
...
class AutoTransferBatchItemProcessorRealExecuteTest extends IntegrationTestSupport {
    ...
    void cleanUp() {
        transactionTemplate().executeWithoutResult(status -> {
            ...
            entityManager
                    .createNativeQuery("DELETE FROM account WHERE customer_id = :id")
                    .setParameter("id", customerId)
                    .executeUpdate();
            entityManager
                    .createNativeQuery("DELETE FROM customer WHERE customer_id = :id")
                    .setParameter("id", customerId)
                    .executeUpdate();
        });
    }
