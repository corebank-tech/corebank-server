...
class SignupAccountVerificationControllerTest {
    ...
    @MockitoBean
    VerifySignupAccountUseCase useCase;

    @Test
    @DisplayName("인증과 CSRF 없이 실명·계좌 인증 토큰을 발급한다")
    void verifiesAccountWithoutAuthenticationOrCsrf() throws Exception {
        given(useCase.verify(any())).willReturn(new VerifySignupAccountResult("ACCOUNT_AUTH_test-token", 600L));

        mockMvc.perform(validRequest())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("0000"))
                .andExpect(jsonPath("$.data.accountAuthToken").value("ACCOUNT_AUTH_test-token"))
    ...
    void alreadyRegisteredExistingBankCustomerReturnsConflict() throws Exception {
        given(useCase.verify(any())).willThrow(new BusinessException(SignupErrorCode.DUPLICATE_EXISTING_BANK_CUSTOMER));
    ...
    void informationMismatchReturnsNoAttempts() throws Exception {
        given(useCase.verify(any())).willThrow(AccountVerificationFailedException.informationMismatch());
    ...
    void passwordMismatchReturnsAttempts() throws Exception {
        given(useCase.verify(any())).willThrow(AccountVerificationFailedException.passwordMismatch(3, 2));
    ...
    void lockedAccountReturnsAttempts() throws Exception {
        given(useCase.verify(any())).willThrow(AccountVerificationFailedException.locked(5, 0));
