...
class SignupIdentityTokenRedisAdapterTest extends IntegrationTestSupport {
    ...
    @Autowired
    UserIdCheckTokenRedisAdapter userIdTokenAdapter;

    @Autowired
    EmailVerificationTokenRedisAdapter emailTokenAdapter;
    ...
    void userIdTokenHasTtlAndIsSingleUse() {
        ...
        UserIdCheckTokenPayload payload = new UserIdCheckTokenPayload("user1234", LocalDateTime.of(2026, 8, 19, 10, 0));

        userIdTokenAdapter.save(token, payload, Duration.ofMinutes(3));

        assertThat(redisTemplate.getExpire("signup:user-id-check:" + token, TimeUnit.SECONDS))
                .isBetween(175L, 180L);
    ...
    void emailTokenHasTtlAndIsSingleUse() {
        ...
        EmailVerificationTokenPayload payload = new EmailVerificationTokenPayload(
                "user@example.com", EmailVerificationPurpose.SIGN_UP, LocalDateTime.of(2026, 8, 19, 10, 0));

        emailTokenAdapter.save(token, payload, Duration.ofMinutes(30));

        assertThat(redisTemplate.getExpire("signup:email-verification:" + token, TimeUnit.SECONDS))
