...
class SignupTermsPersistenceAdapterTest extends IntegrationTestSupport {
    ...
    void findsLatestSignupTerms() {
        ...
        assertThat(result)
                ...
                .contains("TERMS_SERVICE", "TERMS_PRIVACY", "TERMS_MARKETING");
    }

    @Test
    @DisplayName("같은 termsCode에서는 생성시각이 가장 최신인 약관만 조회한다")
    void findsOnlyLatestVersionOfSameTermsCode() {
        insertTerms("TERMS_LATEST_TEST", "v1.0", "SIGNUP", LocalDateTime.of(2026, 8, 18, 0, 0));
        insertTerms("TERMS_LATEST_TEST", "v2.0", "SIGNUP", LocalDateTime.of(2026, 8, 19, 0, 0));
    ...
    void excludesProductTerms() {
        insertTerms("TERMS_PRODUCT_EXCLUSION_TEST", "v1.0", "PRODUCT", LocalDateTime.of(2026, 8, 19, 0, 0));

        List<SignupTerm> result = adapter.findLatestSignupTerms();

        assertThat(result).extracting(SignupTerm::termsCode).doesNotContain("TERMS_PRODUCT_EXCLUSION_TEST");
    }

    private void insertTerms(String termsCode, String version, String termsType, LocalDateTime createdAt) {
        jdbcTemplate.update(
                """
                        INSERT INTO terms (
                            terms_code,
