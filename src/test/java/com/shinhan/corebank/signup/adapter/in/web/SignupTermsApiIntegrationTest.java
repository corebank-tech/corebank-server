...
class SignupTermsApiIntegrationTest extends IntegrationTestSupport {
    ...
    void getSignupTermsWithoutAuthentication() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/auth/terms")
                ...
                .andExpect(jsonPath("$.data.items.length()")
                        .value(currentTerms.size()))
                .andExpect(jsonPath("$.data.items[0].termsId")
                        .isString());
    }
    ...
    void checkTermsIssuesAndStoresTermsAuthToken() throws Exception {
        List<Map<String, Object>> agreements =
                signupTermsQueryPort.findLatestSignupTerms().stream()
                        .map(this::agreement)
                        .toList();

        String requestBody = objectMapper.writeValueAsString(
                Map.of("agreedTerms", agreements)
        );

        String responseBody = mockMvc.perform(
                        post("/api/v1/auth/terms/check")
                                .contextPath("/api/v1")
        ...
        String token = response.get("data")
                .get("termsAuthToken")
                .asText();
