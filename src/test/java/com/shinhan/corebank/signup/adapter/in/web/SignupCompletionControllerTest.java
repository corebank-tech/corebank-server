...
class SignupCompletionControllerTest {
    ...
    void completesWithoutAuthenticationOrCsrf() throws Exception {
        given(idempotencyService.beginAnonymous(
                anyString(), anyString(), anyString()
        )).willReturn(IdempotencyResult.proceed());
        given(completeSignupUseCase.complete(any())).willReturn(
                new CompleteSignupResult(
                        101L,
                        "honggildong",
    ...
    void replaysCompletedResponseWithoutExecutingSignupAgain() throws Exception {
        given(idempotencyService.beginAnonymous(
                anyString(), anyString(), anyString()
        )).willReturn(IdempotencyResult.replay(
                (short) 200,
                """
        ...
        verify(idempotencyService).beginAnonymous(
                anyString(), anyString(), contains("TEMP_SIGNUP_test")
        );
    }
}
