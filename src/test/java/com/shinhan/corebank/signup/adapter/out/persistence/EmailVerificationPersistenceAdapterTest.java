...
class EmailVerificationPersistenceAdapterTest extends IntegrationTestSupport {
    ...
    void savesVerificationRequest() {
        ...
        EmailVerificationJpaEntity saved = repository.findById(requestId).orElseThrow();
    ...
    void invalidatesOnlyMatchingActiveRequests() {
        LocalDateTime now = LocalDateTime.of(2026, 8, 19, 10, 0);
        EmailVerificationRequest request = EmailVerificationRequest.issue(
                requestId(), EmailVerificationPurpose.SIGN_UP, "reissue@example.com", "hash", now.plusMinutes(3), now);
        requestPort.save(request);

        assertThat(requestPort.invalidateActive("reissue@example.com", EmailVerificationPurpose.SIGN_UP))
                .isEqualTo(1);
        assertThat(requestPort.findByIdForUpdate(request.verificationRequestId()))
                .get()
                .extracting(EmailVerificationRequest::used)
                .isEqualTo(true);
    }
