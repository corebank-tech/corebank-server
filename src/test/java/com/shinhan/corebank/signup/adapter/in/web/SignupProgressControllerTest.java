...
class SignupProgressControllerTest {
    ...
    @MockitoBean ValidateSignupUseCase validateSignupUseCase;
    @MockitoBean GetSignupConfirmationUseCase confirmationUseCase;

    @Test
    void validatesWithoutAuthenticationOrCsrf() throws Exception {
        given(validateSignupUseCase.validate(any())).willReturn(
                new ValidateSignupResult("TEMP_SIGNUP_token", 1800L)
        );
        ...
        mockMvc.perform(post("/api/v1/auth/signup/validate")
                ...
                .andExpect(jsonPath("$.data.tempSignupToken")
                        .value("TEMP_SIGNUP_token"))
    ...
    void returnsMaskedConfirmationWithoutAuthentication() throws Exception {
        given(confirmationUseCase.getConfirmation("TEMP_SIGNUP_token"))
                .willReturn(new SignupConfirmationResult(
                        "홍*동", "honggildong", "90.01.01",
                        "010-****-5678", "hon*@corebank.example.com"
                ));
        ...
        mockMvc.perform(get("/api/v1/auth/signup/confirm-info")
                ...
                .andExpect(jsonPath("$.data.email")
                        .value("hon*@corebank.example.com"));
    }

    @Test
    void missingSignupHeaderReturnsCmn0001() throws Exception {
        given(confirmationUseCase.getConfirmation(isNull())).willThrow(
                new BusinessException(CommonErrorCode.INVALID_INPUT)
        );
