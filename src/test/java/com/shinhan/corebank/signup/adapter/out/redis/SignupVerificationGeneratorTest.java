...
class SignupVerificationGeneratorTest {
    ...
    void generatesSecureVerificationRequestId() {
        String id = new SecureVerificationRequestIdGenerator()
                .generateEmailVerificationId();

        assertThat(id).startsWith("EVF_");
        assertThat(Base64.getUrlDecoder().decode(id.substring(4)))
                .hasSize(32);
    }
    ...
    void generatesSixDigitCode() {
        SecureEmailVerificationCodeGenerator generator =
                new SecureEmailVerificationCodeGenerator();
