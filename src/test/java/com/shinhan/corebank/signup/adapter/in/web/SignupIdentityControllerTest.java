...
class SignupIdentityControllerTest {
    ...
    @MockitoBean CheckUserIdUseCase checkUserIdUseCase;
    @MockitoBean IssueEmailVerificationUseCase issueUseCase;
    @MockitoBean VerifyEmailUseCase verifyUseCase;

    @Test
    @DisplayName("인증과 CSRF 없이 아이디 중복확인 토큰을 발급한다")
    void checksUserIdWithoutAuthenticationOrCsrf() throws Exception {
        given(checkUserIdUseCase.check(any())).willReturn(
                new CheckUserIdResult(true, "USER_ID_CHECK_token", 180)
        );
        ...
        mockMvc.perform(post("/api/v1/auth/check-id")
                ...
                .andExpect(jsonPath("$.data.userIdCheckToken")
                        .value("USER_ID_CHECK_token"))
    ...
    void duplicateUserIdReturnsContractError() throws Exception {
        given(checkUserIdUseCase.check(any())).willThrow(
                new BusinessException(SignupErrorCode.DUPLICATE_USER_ID)
        );
    ...
    void issuesEmailVerificationWithoutAuthenticationOrCsrf()
            throws Exception {
        given(issueUseCase.issue(any())).willReturn(
                new IssueEmailVerificationResult(
                        "EVF_request",
                        "012345",
                        180
                )
        ...
        mockMvc.perform(post("/api/v1/auth/email-verifications")
                        ...
                        .content("""
                                ...
                                """))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.emailVerificationId")
                        .value("EVF_request"))
                .andExpect(jsonPath("$.data.verificationCode")
                        .value("012345"))
                .andExpect(jsonPath("$.data.expiresIn").value(180));
    }
    ...
    void verifiesEmailWithoutAuthenticationOrCsrf() throws Exception {
        given(verifyUseCase.verify(any())).willReturn(new VerifyEmailResult(
                "EMAIL_VERIFICATION_token",
                OffsetDateTime.parse("2026-08-19T10:00:00+09:00")
        ));

        mockMvc.perform(post(
                        "/api/v1/auth/email-verifications/EVF_request/verify"
                )
                        .contextPath("/api/v1")
                        .contentType(APPLICATION_JSON)
                        .content("{\"verificationCode\":\"012345\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.emailVerificationToken")
                        .value("EMAIL_VERIFICATION_token"))
                .andExpect(jsonPath("$.data.verifiedAt")
                        .value("2026-08-19T10:00:00+09:00"));
    }
}
