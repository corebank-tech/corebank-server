...
class CustomerLoginAdapterTest {
    ...
    @BeforeEach
    void setUp() {
        adapter = new CustomerLoginAdapter(
                customerAuthenticationFacade,
                new CustomerAuthenticationMapper()
        );
    }
    ...
    void findByUserId() {
        CustomerAuthenticationData data =
                new CustomerAuthenticationData(
                        1L,
                        "user01",
                        "passwordHash",
                        "홍길동",
        ...
        given(customerAuthenticationFacade.findByUserId("user01"))
                .willReturn(Optional.of(data));

        Optional<LoginCustomer> result =
                adapter.findByUserId("user01");
    ...
    void findByUserIdNotFound() {
        given(customerAuthenticationFacade.findByUserId("unknown"))
                .willReturn(Optional.empty());

        Optional<LoginCustomer> result =
                adapter.findByUserId("unknown");
