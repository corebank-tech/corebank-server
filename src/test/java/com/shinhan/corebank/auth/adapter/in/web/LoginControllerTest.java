...
class LoginControllerTest {
    ...
    void logsInSuccessfully() throws Exception {
        LoginResult loginResult = new LoginResult(1L, "login-user", "홍길동");

        OffsetDateTime sessionExpiresAt = OffsetDateTime.parse("2026-08-16T01:10:00Z");

        given(clientIpResolver.resolve(any(HttpServletRequest.class))).willReturn("203.0.113.10");
        given(loginUseCase.login(new LoginCommand("login-user", PASSWORD, "203.0.113.10")))
                .willReturn(loginResult);
        given(sessionLoginManager.establishSession(
                        any(LoginResult.class), any(HttpServletRequest.class), any(HttpServletResponse.class)))
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.sessionExpiresAt").value("2026-08-16T01:10:00Z"))
                .andExpect(content().string(not(containsString(PASSWORD))))
                .andExpect(content().string(not(containsString(PASSWORD_HASH))));

        verify(sessionLoginManager)
                .establishSession(
                        any(LoginResult.class), any(HttpServletRequest.class), any(HttpServletResponse.class));
        verify(csrfTokenCookieIssuer).rotate(any(HttpServletRequest.class), any(HttpServletResponse.class));
    }

    @Test
    @DisplayName("존재하지 않는 아이디도 비밀번호 불일치와 동일한 형태의 데이터를 반환한다 (REQ-AUTH-023)")
    void returnsLoginFailureWithDecoyAttemptDataForNonexistentUser() throws Exception {
        given(clientIpResolver.resolve(any(HttpServletRequest.class))).willReturn("203.0.113.10");
        given(loginUseCase.login(any(LoginCommand.class)))
                .willThrow(LoginFailedException.invalidCredentials(new LoginAttemptResult(1, 4)));
        ...
        verify(sessionLoginManager, never())
                .establishSession(
                        any(LoginResult.class), any(HttpServletRequest.class), any(HttpServletResponse.class));
        verify(csrfTokenCookieIssuer, never()).rotate(any(HttpServletRequest.class), any(HttpServletResponse.class));
    }
    ...
    void returnsLoginFailureWithAttemptData() throws Exception {
        given(clientIpResolver.resolve(any(HttpServletRequest.class))).willReturn("203.0.113.10");
        given(loginUseCase.login(any(LoginCommand.class)))
                .willThrow(LoginFailedException.invalidCredentials(new LoginAttemptResult(2, 3)));
    ...
    void returnsAccountLocked() throws Exception {
        given(clientIpResolver.resolve(any(HttpServletRequest.class))).willReturn("203.0.113.10");
        given(loginUseCase.login(any(LoginCommand.class))).willThrow(LoginFailedException.accountLocked());
    ...
    void rejectsMissingRequiredFields() throws Exception {
        ...
        verify(clientIpResolver, never()).resolve(any(HttpServletRequest.class));
        verify(loginUseCase, never()).login(any(LoginCommand.class));
    }
