...
class LoginApiIntegrationTest extends IntegrationTestSupport {
    ...
    void createsAuthenticatedSessionAndChangesSessionId()
            throws Exception {
        HttpResponse<String> preLoginResponse = httpClient.send(
                HttpRequest.newBuilder(uri("/products/test-session"))
                        .GET()
                        .build(),
        ...
        assertThat(body.get("data").get("userName").asText())
                .isEqualTo("로그인고객");

        OffsetDateTime sessionExpiresAt = OffsetDateTime.parse(
                body.get("data").get("sessionExpiresAt").asText()
        );
        ...
        assertThat(loginResponse.body())
                ...
                .doesNotContain(currentSessionId);
        ...
        JsonNode protectedBody =
                objectMapper.readTree(protectedResponse.body());
    ...
    void doesNotCreateSessionWhenPasswordIsInvalid()
            throws Exception {
        ...
        Integer failureCount = jdbcTemplate.queryForObject(
                "SELECT login_failure_count FROM customer WHERE customer_id = ?",
                Integer.class,
                customerId
        );
    ...
    void invalidatesSessionAndAuthenticationOnLogout() throws Exception {
        ...
        assertThat(logoutBody.get("message").asText())
                .isEqualTo("로그아웃되었습니다.");
    ...
    private URI uri(String path) {
        return URI.create(
                "http://127.0.0.1:%d/api/v1%s"
                        .formatted(serverPort, path)
        );
    }
