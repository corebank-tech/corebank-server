...
class SessionAccessDeniedHandlerTest {
    ...
    private final SessionAccessDeniedHandler deniedHandler = new SessionAccessDeniedHandler(objectMapper);
    ...
    void writesForbiddenResponse() throws Exception {
        ...
        deniedHandler.handle(new MockHttpServletRequest(), response, new AccessDeniedException("외부에 노출하면 안 되는 권한 예외"));
        ...
        assertThat(body.get("message").asText()).isEqualTo("해당 자원에 접근할 권한이 없습니다.");
        assertThat(body.has("data")).isTrue();
        assertThat(body.get("data").isNull()).isTrue();
        assertThat(response.getContentAsString()).doesNotContain("외부에 노출하면 안 되는 권한 예외");
    }
}
