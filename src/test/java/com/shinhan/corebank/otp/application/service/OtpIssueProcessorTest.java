...
class OtpIssueProcessorTest {
    ...
    @Mock
    OtpCodeGeneratorPort codeGeneratorPort;

    @Mock
    OtpTokenGeneratorPort tokenGeneratorPort;

    @Mock
    OtpCodeHashPort codeHashPort;
    ...
    void expiresExistingAndIssuesNewOtp() {
        OtpIssueProcessor processor = new OtpIssueProcessor(
                ...
                codeHashPort,
                canonicalizerPort,
                new OtpProperties(Duration.ofMinutes(3), Duration.ofMinutes(5), true),
        ...
        IssueOtpResult result = processor.issue(new IssueOtpCommand(1L, OtpTransactionType.IMMEDIATE_TRANSFER, data));

        var ordered = inOrder(requestPort);
        ordered.verify(requestPort).expireActiveRequests(eq(1L), any());
        ArgumentCaptor<OtpVerificationRequest> captor = ArgumentCaptor.forClass(OtpVerificationRequest.class);
