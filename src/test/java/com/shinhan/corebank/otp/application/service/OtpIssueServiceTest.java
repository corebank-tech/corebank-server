...
class OtpIssueServiceTest {
    ...
    @Mock
    OtpIssueLockPort issueLockPort;

    @Mock
    OtpIssueProcessor processor;

    @Test
    @DisplayName("고객별 잠금 안에서 OTP를 발급하고 커밋 이후 잠금을 해제한다")
    void issuesWithinCustomerLock() {
        OtpIssueService service = new OtpIssueService(issueLockPort, processor, new OtpTransactionDataValidator());
        IssueOtpCommand command =
                new IssueOtpCommand(1L, OtpTransactionType.IMMEDIATE_TRANSFER, Map.of("amount", 100_000L));
