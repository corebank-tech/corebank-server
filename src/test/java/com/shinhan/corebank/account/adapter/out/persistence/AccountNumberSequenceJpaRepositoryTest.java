...
class AccountNumberSequenceJpaRepositoryTest extends IntegrationTestSupport {
    ...
    void findsDemandDepositSequenceForUpdate() {
        ...
        Optional<AccountNumberSequenceJpaEntity> result =
                repository.findDemandDepositForUpdate(AccountNumberPolicy.BANK_CODE, AccountType.DEMAND_DEPOSIT);

        // then
        assertThat(result).isPresent();

        AccountNumberSequenceJpaEntity entity = result.orElseThrow();

        assertThat(entity.getBankCode()).isEqualTo(AccountNumberPolicy.BANK_CODE);

        assertThat(entity.getAccountType()).isEqualTo(AccountType.DEMAND_DEPOSIT);

        assertThat(entity.getProductId()).isNull();

        assertThat(entity.getProductPrefix()).isEqualTo(DEMAND_DEPOSIT_PREFIX);
    ...
    void findsTimeDepositSequenceForUpdate() {
        // given
        Long productId = fixture.findProductId("PRD_BASIC_DEP");

        fixture.resetProductAccountSequence(productId, AccountType.TIME_DEPOSIT, TIME_DEPOSIT_PREFIX, 3L);

        // when
        Optional<AccountNumberSequenceJpaEntity> result = repository.findProductAccountForUpdate(
                AccountNumberPolicy.BANK_CODE, AccountType.TIME_DEPOSIT, productId);
        ...
        assertThat(entity.getAccountType()).isEqualTo(AccountType.TIME_DEPOSIT);

        assertThat(entity.getProductId()).isEqualTo(productId);

        assertThat(entity.getProductPrefix()).isEqualTo(TIME_DEPOSIT_PREFIX);

        assertThat(entity.getLastSequence()).isEqualTo(3L);
    }
    ...
    void returnsEmptyWhenSequenceDoesNotExist() {
        ...
        fixture.deleteProductAccountSequence(productId, AccountType.TIME_DEPOSIT);
