...
class AccountPasswordChangeAuthVerificationAdapterTest {
    ...
    private final OtpAuthTokenVerifier otpVerifier = mock(OtpAuthTokenVerifier.class);
    private final AccountPasswordChangeAuthVerificationAdapter adapter =
            new AccountPasswordChangeAuthVerificationAdapter(passwordVerifier, otpVerifier);
    ...
    void verifiesOtpToken() {
        ...
        verify(otpVerifier)
                .verifyAndConsume(new OtpAuthTokenVerification(
                        "otp-token", 1L, OtpTransactionType.ACCOUNT_PASSWORD_CHANGE, Map.of("accountId", 101L)));
    }
}
