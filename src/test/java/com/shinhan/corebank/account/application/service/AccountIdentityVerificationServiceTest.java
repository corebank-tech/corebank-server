...
class AccountIdentityVerificationServiceTest {
    ...
    void verifiesOwnedAccountPassword() {
        ...
        given(persistencePort.findByAccountNumber("110550051877"))
                .willReturn(Optional.of(account));
        given(processor.verify(2L, 101L, "1234"))
                .willReturn(new AccountPasswordAttemptResult(
                        101L, true, 0, 5, false
                ));

        AccountIdentityVerificationResult result = service.verify(
                verification(Set.of(1L, 2L))
        );

        assertThat(result.status())
                .isEqualTo(AccountIdentityVerificationStatus.VERIFIED);
    ...
    void hidesMissingOrForeignAccount() {
        ...
        given(persistencePort.findByAccountNumber("110550051877"))
                .willReturn(Optional.of(foreignAccount));
        ...
        assertThat(result.status()).isEqualTo(
                AccountIdentityVerificationStatus.INFORMATION_MISMATCH
        );
    ...
    void returnsPasswordAttemptStatus() {
        ...
        AccountIdentityVerificationResult result = service.verify(
                verification(Set.of(1L))
        );

        assertThat(result.status())
                .isEqualTo(AccountIdentityVerificationStatus.LOCKED);
    }
    ...
    void returnsPasswordMismatchBeforeLock() {
        ...
        assertThat(result.status()).isEqualTo(
                AccountIdentityVerificationStatus.PASSWORD_MISMATCH
        );
    }
    ...
    private AccountIdentityVerification verification(Set<Long> customerIds) {
        return new AccountIdentityVerification(
                customerIds,
                "110550051877",
                "1234"
        );
