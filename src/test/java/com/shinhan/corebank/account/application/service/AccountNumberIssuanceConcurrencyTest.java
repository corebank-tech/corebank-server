...
class AccountNumberIssuanceConcurrencyTest extends IntegrationTestSupport {
    ...
    void issuesUniqueAccountNumbersUnderConcurrency() throws Exception {
        ...
        for (int i = 0; i < REQUEST_COUNT; i++) {
            futures.add(executor.submit(() -> {
                ...
                return service.issue(AccountType.DEMAND_DEPOSIT, null);
            }));
        }
        ...
        for (Future<String> future : futures) {
            accountNumbers.add(future.get(60, TimeUnit.SECONDS));
        }

        // then
        assertThat(accountNumbers).hasSize(REQUEST_COUNT).doesNotHaveDuplicates();

        assertThat(accountNumbers)
                .allMatch(number -> ACCOUNT_NUMBER_PATTERN.matcher(number).matches());
        assertThat(accountNumbers)
                .allMatch(number -> number.startsWith(AccountNumberPolicy.BANK_CODE + DEMAND_DEPOSIT_PREFIX));

        assertThat(accountNumbers)
                .contains(
                        AccountNumberSequenceTestFixture.accountNumberOf(DEMAND_DEPOSIT_PREFIX, 1L),
                        AccountNumberSequenceTestFixture.accountNumberOf(DEMAND_DEPOSIT_PREFIX, REQUEST_COUNT));

        assertThat(fixture.findDemandDepositLastSequence()).isEqualTo(100L);
    }
