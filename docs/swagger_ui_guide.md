...
| 환경 | Swagger UI | API Docs (JSON) | API Docs (YAML) |
| --- | --- | --- | --- |
| 로컬 | `http://localhost:8080/api/v1/swagger-ui/index.html` | `http://localhost:8080/api/v1/v3/api-docs` | `http://localhost:8080/api/v1/v3/api-docs.yaml` |
| 배포 서버 | `https://api.corebank.cloud/api/v1/swagger-ui/index.html` | `https://api.corebank.cloud/api/v1/v3/api-docs` | `https://api.corebank.cloud/api/v1/v3/api-docs.yaml` |
...
`/swagger-ui.html`과 `/v3/api-docs.yaml`을 따로 적는 이유는 같다. `/**`는 **슬래시로 구분된 하위 경로만** 받으므로, `/swagger-ui/**`는 `/swagger-ui.html`을, `/v3/api-docs/**`는 `/v3/api-docs.yaml`을 잡지 못한다. 둘 다 형제 경로라 각각 명시해야 한다. 실제로 `/v3/api-docs.yaml`은 이 누락 때문에 401을 반환하던 이력이 있다(#314).

## 3. 인증이 필요한 API를 Swagger UI에서 호출하기

위 네 경로만 `permitAll`이고, 인증 대상 API는 유효한 로그인 세션이 필요하다. Swagger UI에서 아래 순서로 같은 origin의 API를 호출한다.
