...
| `CMN0006` | 400  | 전체조회 결과가 너무 많습니다. 조회기간을 좁혀 주세요.           | `all=true` 조회 결과가 상한(100건)을 초과              | —                                       |
...
| `ATH0009` | 400 | 실명 또는 계좌 정보가 일치하지 않습니다. | 실명·계좌 인증 실패 또는 아이디 찾기의 계좌비밀번호 불일치 |
| `ATH0010` | 400 | 현재 비밀번호가 일치하지 않습니다. | 로그인 사용자의 비밀번호 변경 시 현재 비밀번호 불일치 |
| `ATH0101` | 401 | 아이디 또는 비밀번호가 일치하지 않습니다. | 로그인 실패. 계정 존재 여부 비노출(REQ-AUTH-023)을 위해 아이디 미존재·비밀번호 불일치 모두 `data`에 `errorCount`·`remainingAttempts` 포함(미존재는 위장값) |
| `ATH0102` | 403 | 비밀번호 5회 오류로 계정이 잠겼습니다. | 로그인 비밀번호 5회 오류에 따른 고객 계정 잠금 또는 계좌비밀번호 5회 오류에 따른 계좌 잠금 |
...
| `ATH0303` | 409 | 이미 가입된 고객입니다. 아이디 찾기를 이용해 주세요. | 원장 고객 기준 재가입 시도. 실명·계좌 인증(A-03)과 회원가입 완료 양쪽에서 반환 |
...
| `ACC0304` | 409 | 이미 등록된 기존 은행 계좌입니다. | 회원가입 시 등록하려는 계좌번호가 이미 존재 |
...
| `LMT9001` | 500 | 이체한도 정보를 확인할 수 없습니다. | 한도 행이 없는 고객의 한도 조회·변경 또는 이체 요청. 가입 연계(REQ-TRSF-029)와 백필이 보장하므로 나오면 데이터 결함이다 |
...
| `PRD9003` | 500 | 가입 건에 연결된 계좌 정보를 찾을 수 없습니다. | 가입 건의 `withdrawal_account_id`가 가리키는 계좌 없음(FK·상위 검증으로 정상 흐름에선 도달 불가) |
...
| `SCD0304` | 409 | 이미 취소된 예약이체입니다. | ⚠️ 더 이상 이 코드로 던져지지 않음. 취소 재요청은 200 + 해당 건 `status: SUCCESS`(멱등 성공)로 확정됨(#330, 다건 취소 API) — 코드 값 자체는 과거 호환을 위해 유지 |
...
| `AUT0010` | 400 | 종료일은 입금계좌의 만기일을 초과할 수 없습니다.                        | 정기적금 입금계좌 만기일 검증(#418), 등록·변경 시점 공통 |
...
| `AUT0302` | 409 | 정상 상태가 아닌 자동이체입니다.                                 | 종료·해지 건 변경·해지 시도. 단 다건 해지에서 이미 해지된 건(`TERMINATED`) 재요청은 200 + `status: SUCCESS`(멱등 성공)로 처리하고, 기간 만료(`EXPIRED`)만 이 코드로 응답한다(#330) |
...
목록 조회 API는 `all`(boolean, 기본값 `false`) 쿼리 파라미터를 공통으로 받는다. `true`면 `page`·`size`를 무시하고 조회조건에 해당하는 전체 건을 반환하며, 응답은 `page: 0`, `size: totalCount`, `totalPages: 1`로 채워진다. `common/util/PageableResolver`가 이 판단(검증 스킵 + `Pageable.unpaged()` 반환)을 공통으로 처리하므로, 새 목록 API를 추가할 때도 파라미터 이름을 `all`로 통일한다. 조회조건이 넓어 결과가 과도하게 많아질 수 있는 API는 도메인별로 결과 건수 상한을 두고 `CMN0006`으로 거부할 수 있다(예: 예약이체 목록조회, 상한 100건).
