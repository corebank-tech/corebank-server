...
| 코드 | HTTP | 메시지 | 의미 |
| --- | --- | --- | --- |
| `ATH0001` | 400 | 비밀번호 규칙에 맞지 않습니다. | 영문자(대·소문자 구분 없음)·숫자·특수문자를 각각 1자 이상 포함하지 않거나, 로그인 아이디 포함·동일 문자 4회 연속·연속 증가/감소 숫자 4자리 금칙 규칙 위반 |
...
| `ATH0102` | 403 | 비밀번호 5회 오류로 계정이 잠겼습니다. | 로그인 비밀번호 5회 오류에 따른 고객 계정 잠금 |
...
> 회원가입·비밀번호 재설정·로그인 사용자의 비밀번호 변경에 사용하는 로그인 비밀번호는 8~15자이며, 영문자(대문자 또는 소문자)·숫자·특수문자를 각각 1자 이상 포함해야 합니다. 로그인 아이디 포함, 동일 문자 4회 연속, 연속 증가·감소 숫자 4자리는 허용하지 않습니다.

> 로그인 사용자의 비밀번호 변경이 성공하면 새 비밀번호를 기준으로 로그인 실패를 다시 집계하도록 `login_failure_count`를 0으로 초기화합니다.
...
| `OTP0103` | 403 | OTP 오류 횟수(5회)에 도달하여 잠금 처리되었습니다. OTP를 재발급받아 주세요. | 5회 오입력 시 요청 잠금 |
...
| `APW0003` | 400 | 직전 계좌비밀번호는 재사용할 수 없습니다. | 변경 시 현재 계좌비밀번호와 동일한 숫자 4자리 재사용 |
...
| `accountAuthToken` (`POST /auth/verify-account`) | 600초 | 회원가입 입력검증                                                |
...
- `accountPasswordAuthToken`은 반드시 `ACC_PWD_` 접두어를 사용합니다.
...
- 동일한 `이메일 + 인증 목적(SIGN_UP 또는 EMAIL_CHANGE)`의 발급 요청은 해당 조합을 기준으로 직렬화합니다.
- 직렬화 구간 안에서 기존 활성 요청을 무효화한 뒤 신규 요청을 저장합니다.
- 동시 발급 요청이 여러 개여도 마지막으로 처리된 요청 하나만 활성 상태여야 합니다.
- 먼저 발급된 인증번호는 사용할 수 없으며, 인증 목적이 다른 요청은 서로 무효화하지 않습니다.

**복수 인증 토큰 소비 실패**

- 계좌비밀번호 인증 토큰과 OTP 인증 토큰은 순차적으로 검증하고 소비합니다.
- 앞선 토큰이 소비된 뒤 다음 토큰 검증이나 업무 처리가 실패해도 소비된 토큰은 복구하지 않습니다.
- 최종 업무 요청이 실패하면 클라이언트는 남은 토큰도 폐기하고 계좌비밀번호 인증과 OTP 인증을 모두 다시 수행해야 합니다.
- 소비된 토큰을 되살리지 않는 fail-closed 정책으로 토큰 재사용과 복구 과정의 동시성 위험을 방지합니다.

**고객 단위 계좌비밀번호 인증 토큰 소비**

- 이체한도 변경처럼 고객 단위로 처리하는 업무도 토큰에 저장된 `accountId`의 소유권과 현재 계좌비밀번호 잠금 상태를 소비 직전에 다시 확인합니다.
- 토큰 발급 후 해당 계좌가 잠겼다면 토큰을 소비하거나 업무를 처리하지 않고 `APW0101`을 반환합니다.
- 토큰의 고객 또는 계좌가 요청 주체와 일치하지 않으면 `APW0102`를 반환합니다.
...
> **고객정보 변경 예외**: `PATCH /customers/me`의 `emailVerificationToken`은 멱등 지문에 포함합니다. 동일한 고객정보 변경 내용이라도 이메일 인증 토큰이 달라지면 서로 다른 요청으로 판단하며, 같은 `Idempotency-Key`를 사용하면 `CMN0302`를 반환합니다.
...
- `PATCH /customers/me`에 전달된 휴대폰 번호와 이메일이 모두 현재 저장값과 같으면 변경할 내용이 없는 요청으로 보고 `CMN0001`을 반환합니다.
- 이메일을 실제로 변경할 때만 `emailVerificationToken`을 요구하고 소비합니다.
- 휴대폰 번호만 변경하는 요청에는 이메일 인증 토큰이 필요하지 않습니다.
...
| 로그인 비밀번호 변경 | 변경 성공 시 해당 고객의 현재 세션을 포함한 모든 로그인 세션을 무효화하고 새 비밀번호로 재로그인 | 보안 정책 |
...
로그인 비밀번호 변경 감사 로그는 다음 기준을 적용합니다.

| 결과 | 감사 정책 |
| --- | --- |
| 필수값 누락·JSON 문법 오류·비밀번호 확인 불일치 등 입력 형식 오류 | 기록하지 않음 |
| 현재 비밀번호 불일치(`ATH0010`) | 보안 실패 이벤트 기록 |
| 잠긴 계정의 변경 요청(`ATH0102`) | 보안 실패 이벤트 기록 |
| 비밀번호 변경 성공 | 성공 이벤트 기록 |

감사 로그에는 고객ID·요청일시·IP·거래유형·결과·실패 오류코드를 기록하며, 현재 비밀번호·신규 비밀번호·확인값 및 비밀번호 해시는 기록하지 않습니다.
