...
> `CMN0001`과 `CMN9999`는 `GlobalExceptionHandler`(→`ApiExceptionHandler`)가 자동 반환하므로 직접 던지지 않아도 됩니다.
...
| 코드 | HTTP | 메시지 | 의미 |
| --- | --- | --- | --- |
| `LMT0001` | 400 | 출금가능금액이 부족합니다. | 잔액 부족 (기존 `TRF0007` 이관) |
...
| `LMT0005` | 404 | 계좌를 찾을 수 없거나 접근할 수 없습니다. | 미존재와 타인 계좌를 구분하지 않음(§8-3) |
...
| `AUT0002` | 400 | 이체 종료일은 시작일 이후, 60개월 이내여야 합니다. | REQ-AUTO-004 |
...
**적용 대상**: 계좌 상세 · 거래내역 · 거래내역 CSV · 계좌별명 관리 · 출금계좌 관리 · 계좌비밀번호 검증·변경 · 이체한도 조회·변경 · 자동이체 조회·변경·해지
