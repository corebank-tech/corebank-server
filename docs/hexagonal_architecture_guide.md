...
## 🧪 통합 테스트에서 협력 객체를 모킹해도 되는가 (#363)

`IntegrationTestSupport`(`@SpringBootTest` + Testcontainers) 기반 테스트에서 `@MockitoBean`을 쓰는 건 아래 세 경우로 한정합니다. 그 외의 이유(그냥 편해서, 기존 코드 복사)로는 모킹하지 않습니다.

1. **도메인 경계 테스트**: 다른 도메인의 `api/` 인터페이스(예: `OtpAuthTokenVerifier`)를 올바르게 호출하는지만 볼 때. 그 인터페이스 구현 자체의 정확성은 소유 도메인의 테스트가 책임집니다.
2. **결과 강제 유발**: 실패·롤백·재시도 분기처럼 실제 협력 객체로는 결정론적으로 재현할 수 없는 결과를 주입해야 할 때.
3. **컨트롤러 계층 격리**: 같은 도메인이라도 그 로직을 검증하는 서비스 테스트가 따로 있어서,
   컨트롤러의 입력 검증·응답 매핑만 떼어내 보려는 경우.

세 경우 모두 **모킹한 필드(사유가 같으면 필드 묶음) 위에 왜 모킹했는지 한 줄 주석**을 남깁니다. 어디에도 해당하지 않으면 실제 협력 객체를 그대로 씁니다.
