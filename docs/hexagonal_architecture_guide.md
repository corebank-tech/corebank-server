...
`IntegrationTestSupport`(`@SpringBootTest` + Testcontainers) 기반 테스트에서 `@MockitoBean`을 쓰는 건 아래 두 경우로 한정합니다. 그 외의 이유(그냥 편해서, 기존 코드 복사)로는 모킹하지 않습니다.
...
두 경우 모두 **모킹한 필드 위에 왜 모킹했는지 한 줄 주석**을 남깁니다. 어느 쪽에도 해당하지 않으면 실제 협력 객체를 그대로 씁니다.
